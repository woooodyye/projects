...
Ray Camera::generate_ray(double x, double y) const {
  ...
  float aspectRatio = screenW / (float) (screenH);


  double ycamera =  y  * tan(radians(vFov/2)) *2 ;

  double xcamera =  x  * tan(radians(hFov/2) ) *2;


  Vector3D direction = c2w * Vector3D(xcamera , ycamera, -1);
