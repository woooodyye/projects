...
float Vertex::laplacian() const {
  ...
  //loop around the vertex to calculate laplacian
  HalfedgeCIter h = halfedge();
  ...
  do{
    Vector3D j = h->twin()->vertex()->position;

    Vector3D b = h->next()->next()->vertex()->position;
    Vector3D a = h->twin()->next()->next()->vertex()->position;
    Vector3D ai = i -a ;
    Vector3D aj = j -a ;
    ...
    double cota = dot(ai,aj) / (cross(ai,aj)).norm() ;
    double cotb = dot(bi,bj) / (cross(bi,bj)).norm() ;

    laplace += (cota + cotb) * ( h->twin()->vertex()->offset - offset);
