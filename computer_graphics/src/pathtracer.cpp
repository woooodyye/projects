  ...
  if (!isect.bsdf->is_delta()) {
    ...
    for (SceneLight* light : scene->lights) {
      ...
      for (int i = 0; i < num_light_samples; i++) {
        ...
        Ray * back = new Ray(hit_p + EPS_D * isect.n , dir_to_light);

        back->max_t = dist_to_light;
//
        if(bvh->intersect(*back, &isect)){
          continue;
        }
  ...
  }
  {
    ...
    float terminateP = 1.f - clamp(L_r.illum(),0.f, 1.f);

    if(p < terminateP){
      return L_out;
    }

    Ray indirect = Ray(hit_p + EPS_D *  wi, wi);

    indirect.depth = r.depth + 1;

    return L_out  +   (L_r * trace_ray(indirect) ) * (cos_theta  / ( pdf * (1.f -terminateP) ))  ;
  }
    //return L_out;
}
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  Spectrum ave = Spectrum();


  for(int i=0; i< num_samples; i++){
    Vector2D point = gridSampler->get_sample();
    px = (x + point.x) / (float) w;
    py = (y + point.y) / (float) h;

    ave += trace_ray(camera->generate_ray(px,py));

  }
  return (1.f / num_samples ) * ave;
}
