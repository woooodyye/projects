...
// The real calculation, calculate the jacobian given a goaljoint and a target position.
Vector3D Joint::calculateJacobian(Joint* goalJoint, Vector3D q){
    Vector3D p = goalJoint->getEndPosInWorld() - this->getBasePosInWorld();
    vector<Vector3D> axes;
    this->getAxes(axes);

    // calculate jacobian for each axis.
    ...
    gradx = dot(jx, (p_theta-q));
    grady = dot(jy, (p_theta-q));
    gradz = dot(jz, (p_theta-q));
    return Vector3D(gradx, grady, gradz);
}

void Joint::calculateAngleGradient(Joint* goalJoint, Vector3D q) {

    //set the angle gradient;
    this->ikAngleGradient =  calculateJacobian(goalJoint, q);

    //recursively calculate the angle graident until it's null;
    if(parent!= nullptr){
        parent->calculateAngleGradient(goalJoint, q);
    }
...
Matrix4x4 Joint::getTransformation() {
  ...
  */
  ...
  Matrix4x4 meshT = skeleton->mesh->getTransformation();
...
Vector3D Joint::getBasePosInWorld() {
    ...
    //base in joint coordinate is 0,0,0;
    Matrix4x4 T = this->getTransformation();
    Vector4D origin = Vector4D(0,0,0,1);
    origin = T * origin;
    return origin.projectTo3D();
}

Vector3D Joint::getEndPosInWorld() {

  Vector3D base =getBasePosInWorld();

  Vector4D end = Vector4D(base, 1);

  //apply the transformation of it self in addition to the base position.
  Vector4D trans = this->getTransformation() * (SceneObject::getTransformation() * Matrix4x4::translation(this->axis) *
          Vector4D(0,0,0,1));
  return trans.projectTo3D();
}
}  // namespace DynamicScene
}  // namespace CMU462
