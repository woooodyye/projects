...
void Mesh::linearBlendSkinning(bool useCapsuleRadius) {
    ...
    for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
        ...
        //create a vector of lbsinfo;
        vector<LBSInfo> lbs;
        lbs.resize(skeleton->joints.size());


        //bind position of the vertex;
        Vector3D vpos = v->bindPosition;


        //loop through all the joints in the skeleton;
        for (size_t i = 0; i < skeleton->joints.size(); i++) {

            Joint *j = skeleton->joints[i];


            //get position of the vertex with respect to the joint in the joint coordinate system;
            Vector3D revjoint = (j->getBindTransformation().inv() * Vector4D(vpos,1)).projectTo3D();

            //vertex position with respect to the world coordinate system
            Vector3D vertexnew = j->getTransformation() * j->SceneObject::getTransformation() * revjoint;

            //find the projection of the vertex onto the joint;
            Vector3D a1 = ( dot(revjoint, j->axis) *  (1 / ( dot(j->axis, j->axis) )  ) ) * j->axis;

            //smallest is the base itself;
            if(dot(revjoint, j->axis) <= 0){
                a1 = Vector3D();
            }

            Vector3D a2 = revjoint - a1;

            //if greater than capsule radius, set it to infinity;
            if (useCapsuleRadius && a2.norm() > j->capsuleRadius) {
                lbs[i].distance = INF_D;
                continue;
            }

            lbs[i].blendPos = vertexnew;
            lbs[i].distance = a2.norm();
        }


        //calculate the total inverse distance. skip if infinity, meaning not into account;
        double wtotal = 0.0;
        for (size_t i = 0; i < lbs.size(); i++) {
            if (lbs[i].distance == INF_D) continue;
            wtotal += 1.0 / lbs[i].distance;

        }

        Vector3D vi = Vector3D();
        //calculate weighted average and the new position.
        for (size_t i = 0; i < lbs.size(); i++) {

            if (lbs[i].distance == INF_D) continue;
            double wij = 1.0 / (lbs[i].distance * wtotal);
            vi += wij * lbs[i].blendPos;
        }

        //set the new vertex position.
        if (vi == Vector3D()) {
          //don't set new positon; vertex is not affected by any joint;
            continue;
        } else {
            v->position = vi;
        }
...
void Mesh::forward_euler(float timestep, float damping_factor) {
    ...
    offsets.resize(mesh.nVertices());

    double i =0;

    //store the update vertex offsets;
    //use the original velocity to update the offset;
    for(VertexIter v= mesh.verticesBegin(); v!= mesh.verticesEnd(); v++){

        float o_pp = v->laplacian() - damping_factor * v->velocity;

        float v_p = v->velocity +  timestep * o_pp;
        float o_p = v->offset + timestep * v->velocity;
    ...
    //update the vertex offsets in the mesh;
    double n=0;
    for(VertexIter v= mesh.verticesBegin(); v!=mesh.verticesEnd(); v++){
        v->offset = offsets[n];
        n+=1;
...
void Mesh::symplectic_euler(float timestep, float damping_factor) {
  ...
  double i =0;
  //use the updated velocities for offsets;
  for(VertexIter v= mesh.verticesBegin(); v!= mesh.verticesEnd(); v++){

      float o_pp = v->laplacian() - damping_factor * v->velocity;

      float v_p = v->velocity +  timestep * o_pp;
      float o_p = v->offset + timestep * v_p;
      v->velocity =   v_p;
      offsets[i] = o_p;
      i+=1;
  }

  //update offsets in the mesh for vertices;
  double n=0;
  for(VertexIter v= mesh.verticesBegin(); v!=mesh.verticesEnd(); v++){
      v->offset = offsets[n];
      n+=1;
