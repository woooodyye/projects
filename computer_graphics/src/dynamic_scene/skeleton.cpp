...
void Skeleton::reachForTarget(map<Joint*, Vector3D> targets, double time) {
   ...
   //do jacobian iteration 100 times to minimize the error;
  typename std::map<Joint*, Vector3D>::iterator j;
//
  for(int i =0; i < 100; i++){

    for(j= targets.begin(); j != targets.end(); j++){
        Joint* targetJ = j->first;  Vector3D q= j->second;
        //calculate the angle gradient in the target joint;
        targetJ->calculateAngleGradient(targetJ, q);
    }

    for(j= targets.begin(); j != targets.end(); j++) {

        //retrieve the original angle
        Vector3D angle = j->first->getAngle(time);
        //set the new angle with the fixed time step. set the fixed timestep of 0.01;
        j->first->setAngle(time, angle - 0.01* j->first->ikAngleGradient);
    }

  }

  // upgrade position of each skeleton and
